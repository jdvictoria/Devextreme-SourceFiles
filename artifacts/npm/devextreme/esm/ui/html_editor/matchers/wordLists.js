...
function getIndent(node, msStyleAttributeName) {
  var style = node.getAttribute(msStyleAttributeName);
  if (style) {
...
var getMatcher = quill => {
  ...
  var msStyleAttributeName = quill.MS_LIST_DATA_KEY;
  return (node, delta) => {
    ...
    var indent = listDecoratorMatches && getIndent(node, msStyleAttributeName);
