...
var ARIA_LABEL_DATE_FORMAT = 'date';
var SELECTION_MODE = {
  single: 'single',
  multiple: 'multiple',
  range: 'range'
...
var BaseView = Widget.inherit({
  ...
  _getSingleModeAriaLabel() {
    ...
    var localizedWidgetName = this._getLocalizedWidgetName();
    var formattedDate = dateLocalization.format(value, ARIA_LABEL_DATE_FORMAT);
    var selectedDatesText = messageLocalization.format('dxCalendar-selectedDate', formattedDate);
    var ariaLabel = "".concat(localizedWidgetName, ". ").concat(selectedDatesText);
    return ariaLabel;
  },
  _getRangeModeAriaLabel() {
    ...
    var formattedStartDate = dateLocalization.format(startDate, ARIA_LABEL_DATE_FORMAT);
    var formattedEndDate = dateLocalization.format(endDate, ARIA_LABEL_DATE_FORMAT);
    var selectedDatesText = startDate && endDate ? messageLocalization.format('dxCalendar-selectedDateRange', formattedStartDate, formattedEndDate) : messageLocalization.format('dxCalendar-selectedDate', formattedStartDate !== null && formattedStartDate !== void 0 ? formattedStartDate : formattedEndDate);
    var ariaLabel = "".concat(localizedWidgetName, ". ").concat(selectedDatesText);
    return ariaLabel;
  },
  _getMultipleModeAriaLabel() {
    var ariaLabel = this._getLocalizedWidgetName();
    return ariaLabel;
  },
  _getTableAriaLabel() {
    var {
    ...
    var isValueEmpty = !value || Array.isArray(value) && !value.filter(Boolean).length;
    if (isValueEmpty) {
  ...
  _updateTableAriaLabel() {
    var label = this._getTableAriaLabel();
    this.setAria({
      label
    }, this._$table);
  },
  ...
  getCellAriaLabel(date) {
    ...
    var isToday = this._isTodayCell(date);
    var format = this._getCurrentDateFormat();
    var dateRangeText = format ? dateLocalization.format(date, format) : this._getCellText(date);
    var ariaLabel = isToday ? "".concat(dateRangeText, ". ").concat(CURRENT_DATE_TEXT[viewName]) : dateRangeText;
    return ariaLabel;
  },
