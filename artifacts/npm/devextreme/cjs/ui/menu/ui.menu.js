...
let Menu = /*#__PURE__*/function (_MenuBase) {
  ...
  _proto._isTargetOutOfComponent = function _isTargetOutOfComponent(relatedTarget) {
    const isInsideRootMenu = (0, _renderer.default)(relatedTarget).closest(".".concat(DX_MENU_CLASS)).length !== 0;
    const isInsideContextMenu = (0, _renderer.default)(relatedTarget).closest(".".concat(DX_CONTEXT_MENU_CLASS)).length !== 0;
  ...
  _proto._focusOutHandler = function _focusOutHandler(e) {
    ...
    _MenuBase.prototype._focusOutHandler.call(this, e);
  };
  ...
  _proto._attachSubmenuHandlers = function _attachSubmenuHandlers($menuAnchorItem, submenu) {
  ...
  _proto._submenuOnShowingHandler = function _submenuOnShowingHandler($menuAnchorItem, submenu, _ref) {
    let {
      rootItem
    } = _ref;
    const $border = $menuAnchorItem.children(".".concat(DX_CONTEXT_MENU_CONTAINER_BORDER_CLASS));
  ...
  _proto._submenuOnShownHandler = function _submenuOnShownHandler($menuAnchorItem, submenu, _ref2) {
    let {
      rootItem
    } = _ref2;
    const params = this._getVisibilityChangeEventParams(rootItem, submenu, $menuAnchorItem);
  ...
  _proto._submenuOnHidingHandler = function _submenuOnHidingHandler($menuAnchorItem, submenu, eventArgs) {
    const $border = $menuAnchorItem.children(".".concat(DX_CONTEXT_MENU_CONTAINER_BORDER_CLASS));
  ...
  _proto._submenuOnHiddenHandler = function _submenuOnHiddenHandler($menuAnchorItem, submenu, _ref3) {
    let {
      rootItem
    } = _ref3;
  ...
  _proto._getVisibilityChangeEventParams = function _getVisibilityChangeEventParams(submenuItem, submenu, $menuAnchorItem, isHide) {
    let itemData;
    let $submenuContainer;
    if (submenuItem) {
      const anchor = isHide ? (0, _renderer.default)(submenuItem).closest(".".concat(DX_MENU_ITEM_CLASS))[0] : submenuItem;
      itemData = this._getItemData(anchor);
      $submenuContainer = (0, _renderer.default)(anchor).find(".".concat(DX_SUBMENU_CLASS)).first();
    } else {
      const $overlayContent = (0, _renderer.default)(submenu._overlay.content());
      itemData = this._getItemData($menuAnchorItem);
      $submenuContainer = $overlayContent.find(".".concat(DX_SUBMENU_CLASS)).first();
    }
    return {
      itemData,
