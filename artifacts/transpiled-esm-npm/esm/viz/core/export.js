...
function getRendererWrapper(width, height, backgroundColor) {
  var rendererContainer = $('<div>').get(0);
...
export var getMarkup = widgets => combineMarkups(widgets).root.outerHTML;
...
export var combineMarkups = function combineMarkups(widgets) {
  ...
  var exportItems = widgets.reduce((r, row, rowIndex) => {
    var rowInfo = row.reduce((r, item, colIndex) => {
      ...
      var node = item.element().find('svg').get(0).cloneNode(true);
  ...
  var backgroundColor = "".concat(exportItems.backgroundColors.length === 1 ? exportItems.backgroundColors[0] : '');
  var {
    totalWidth,
    totalHeight
  } = exportItems;
  var rootElement = wrapItemsToElement(totalWidth, totalHeight, backgroundColor, {
    options,
    exportItems,
    compactView
  });
...
function wrapItemsToElement(width, height, backgroundColor, _ref) {
  ...
  var rendererWrapper = getRendererWrapper(width, height, backgroundColor);
  var getVOffset = item => {
  ...
  exportItems.items.forEach(item => {
    var container = rendererWrapper.createGroup();
    container.attr({
  ...
  var result = rendererWrapper.getRootContent();
